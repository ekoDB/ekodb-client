...
- **Kotlin HTTP errors.** Expose terminal request failures through
  ...
  exception message format, stop retrying terminal client errors, and throw
  on exhausted server errors instead of silently succeeding in collection
  creation. Authentication refresh and rate-limit retries remain supported.
...
- **Stream errors carry the provider-failure classification.** When a chat
  ...
  with `is_provider_failure()`. The payload used to be the bare `String`;
  `ChatStreamError` derefs to `str`, compares to `&str` / `String`, converts
  into `String` and `Box<dyn Error>`, and `Display` is the message, so
...
- **Error responses are no longer assumed to carry a JSON body.** Non-success
  ...
  so `delete_chat_session` and `delete_chat_message` report what actually
  happened. The typed statuses map to the same variants `json_body` and
  `handle_response` produce — 401 → `TokenExpired`, 404 → `NotFound`, 429 →
  `RateLimit` carrying the server's `Retry-After`, 503 → `ServiceUnavailable` —
