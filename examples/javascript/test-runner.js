...
const EXAMPLE_TIMEOUT_SECONDS = Number(
  process.env.EXAMPLE_TIMEOUT_SECONDS || "300",
);
if (
  !Number.isInteger(EXAMPLE_TIMEOUT_SECONDS) ||
  EXAMPLE_TIMEOUT_SECONDS <= 0
) {
  throw new Error("EXAMPLE_TIMEOUT_SECONDS must be a positive integer");
}
...
function runExample(file, token) {
  return new Promise((resolve, reject) => {
    ...
    const timeout = setTimeout(() => {
      timedOut = true;
      log(
        `✗ ${path.basename(file)} timed out after ${EXAMPLE_TIMEOUT_SECONDS}s`,
        "red",
      );
      killGroup("SIGTERM");
      setTimeout(() => {
        killGroup("SIGKILL");
        reject(new Error(`Example timed out: ${file}`));
      }, 5000);
    }, EXAMPLE_TIMEOUT_SECONDS * 1000);

    child.on("exit", (code) => {
      clearTimeout(timeout);
      // A timed-out leader can exit before signal-resistant descendants.
      // The grace-period callback owns settlement after killing the group.
      if (timedOut) return;
      if (code === 0) {
