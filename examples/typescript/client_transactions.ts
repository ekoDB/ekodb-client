import { EkoDBClient, extractRecordId, getValue } from "@ekodb/ekodb-client";
...
const COLLECTION = "client_transactions_accounts_ts";
...
async function main() {
  ...
  try {
    ...
    const aliceId = extractRecordId(alice);
    ...
    const bobId = extractRecordId(bob);
    ...
    if (
      getValue<number>(aliceInTx.balance) !== 800 ||
      getValue<number>(bobInTx.balance) !== 700
    ) {
    ...
    if (
      getValue<number>(committedAlice.balance) !== 800 ||
      getValue<number>(committedBob.balance) !== 700
    ) {
    ...
    if (getValue<number>(bobInRollbackTx.balance) !== 600) {
      throw new Error(
    ...
    if (getValue<number>(bobAfterRollback.balance) !== 700) {
  ...
  for (const transactionId of activeTransactions.slice().reverse()) {
    try {
