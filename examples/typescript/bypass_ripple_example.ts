...
async function main() {
  ...
  const collection = `bypass_ripple_products_ts_${process.pid}_${Date.now()}`;
  ...
  try {
    ...
    try {
      ...
      const notFound =
        message.includes("status 404") || /not found/i.test(message);
      if (!notFound) {
        throw new AggregateError(
          primaryError === undefined ? [error] : [primaryError, error],
          "Bypass ripple example or cleanup failed",
        );
      }
    }
  }
