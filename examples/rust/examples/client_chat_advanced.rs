...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for chat_id in chat_ids.iter().rev() {
        match client.delete_chat_session(chat_id).await {
            Ok(()) => println!("✓ Deleted chat session: {chat_id}"),
            Err(error) => cleanup_errors.push(format!("chat session {chat_id}: {error}")),
        }
    }
